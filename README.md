...
### v1.1.1 (2026-06-23)
- ✅ **新增一次性任务与每日任务划分**：通过绿（每日）、蓝（一次性）圆角垂直颜色带区分；支持已完成任务逆向流转；优化输入框切换药丸按钮为极简圆点图标 (`🟢 ▼`/`🔵 ▼`)。
- ✅ **新增智能排序功能**：标签栏左侧嵌入 `⇅` 排序按钮，**零高度空间开销**还原任务卡片高度；支持“默认排序”与“每日优先”一键切换及状态持久化，配有多语言 Tooltip 悬浮提示。
- ✅ **新增历史记录窗口 (History)**：独立可拖拽、可置顶无边框设计，展示已完成一次性任务；支持长文本自适应自动换行，防止截断。
- ✅ **新增多选点选与批量物理删除**：历史列表改用 `MultiSelection` 模式，被选中行高亮泛绿，标题栏新增精致垃圾桶 `🗑` 按钮（Hover 呈红色警告），点击可安全一键批量删除。
- ✅ **新增自动同步与手动刷新**：历史记录注册全局更新回调，状态改变秒级自动同步；标题栏新增刷新 `↻` 按钮，支持手动强制重载；覆写窗口关闭事件，销毁时自动移除回调注销监听以规避内存泄露。
- ✅ **UI 指向性手型光标优化**：为主窗口和历史记录的所有可点击按钮（标签、控制、添加、单元格行）绑定手型 Pointer 光标（`Qt.PointingHandCursor`），强化点击引导。
- ✅ **锁定列宽排版**：固定历史表“编号”（`55px`）与“完成日期”（`135px`）的列宽不可拖动，完美避免表头截断。

### v1.1.0 (2026-06-21)
- ✅ **新增自动更新检查器和数据库迁移系统**：应用启动时自动检测 GitHub 最新 Release，提供更新提醒与下载跳转；集成安全的 SQLite 数据库迁移框架，支持无缝版本过渡。
